...
def root():
    return {
        ...
        "endpoints": ["/agenda", "/availability", "/book", "/cancel", "/__whoami"],
...
@app.get("/__whoami")
def whoami():
    """Endpoint de diagnóstico para confirmar qué módulo está corriendo en Render."""
...
def agenda(
    ...
    if start and end:
        # datetimes aware con la ZONA_LOCAL
...
def book(payload: dict = Body(...)):
    ...
    """
    try:
        # Si no están habilitadas las escrituras, forzamos dry_run=True
...
if __name__ == "__main__":
    import uvicorn
    # Si corrés local:  uvicorn app:app --reload
